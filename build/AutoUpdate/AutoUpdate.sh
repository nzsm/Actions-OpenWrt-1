...
if [[ ${Stable_Mode} == 1 ]];then
    ...
    GET_FullVersion=$(cat /tmp/Github_Tags | egrep -o "openwrt-${CURRENT_DEVICE}-[0-9]+.[0-9]+.[0-9]+.[0-9]+${GET_Version_Type}+.[a-z]+.[a-z]+" | awk 'END {print}')
    GET_Version="${GET_FullVersion:0-27:20}"
    else
    GET_FullVersion=$(cat /tmp/Github_Tags | egrep -o "openwrt-${CURRENT_DEVICE}-[0-9]+.[0-9]+.[0-9]+.[0-9]+${GET_Version_Type}+.[a-z]+.[a-z]+" | awk 'END {print}')
    GET_Version="${GET_FullVersion:0-24:20}"
    fi
else
	if [[ $CURRENT_DEVICE == x86-64 ]];then
    GET_FullVersion=$(cat /tmp/Github_Tags | egrep -o "openwrt-${CURRENT_DEVICE}-[0-9]+.[0-9]+.[0-9]+.[0-9]+.[a-z]+.[a-z]+" | awk 'END {print}')
    GET_Version="${GET_FullVersion:0-20:13}"
    else
    GET_FullVersion=$(cat /tmp/Github_Tags | egrep -o "openwrt-${CURRENT_DEVICE-}-[0-9]+.[0-9]+.[0-9]+.[0-9]+.[a-z]+.[a-z]+" | awk 'END {print}')
